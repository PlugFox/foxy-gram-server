...
type captchaKeyboard struct {
	...
	backspace tele.InlineButton
	keyboard  [][]tele.InlineButton
}

func captchaKeyboardDefault() captchaKeyboard {
	numbers := make([]tele.InlineButton, 0, 10) //nolint:mnd
	...
	backspace := tele.InlineButton{Text: "↩️", Unique: captchaKeyboardUnique, Data: "captcha-backspace"}

	keyboard := [][]tele.InlineButton{
		{numbers[1], numbers[2], numbers[3]},
		{numbers[4], numbers[5], numbers[6]},
		{numbers[7], numbers[8], numbers[9]},
		{refresh, numbers[0], backspace},
	}
